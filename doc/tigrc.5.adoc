...
'quick-options' (string)::

	A string to specify the rev range that will be passed to the git-log(1)
	command by the quick view. Register specification can be used so that
	the rev range can take the register value at runtime. Options can be
	overridden by command line options.

'refs-options' (string)::

	A string to specify the rev range that will be passed to the git-log(1)
	command by the refs view. Register specification can be used so that
	the rev range can take the register value at runtime. Options can be
	overridden by command line options.
...
|%(quickargs)		|Options from 'quick-options' used by the quick view.
|%(refsargs)		|Options from 'refs-options' used by the refs view.
